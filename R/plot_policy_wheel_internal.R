plot_policy_wheel_internal <- function(y, states, df, wheel_opts, policies, font_size){
  ...
  for(s in unique(states)) {
    highlight.sector(sector.index = s, track.index = 1,
                     text = s, padding = c(-.5,1,1.5,1), cex = 1.3*font_size, text.vjust = .5, col = NA, facing = "downward")
  }
  
  # Label each graph with the correct year (in the middle of the circles)
  text(0,0, labels = print(y), cex = 2.8*font_size)
  ...
  highlight.sector(sector.index = c("CT", "ME", "MA", "NH", "RI", "VT"), track.index = 1,
                   text = "New England", padding = c(1,0,5,0), cex = 1.4*font_size, font = 2,
                   border = NA, col = NA, facing = "bending.inside")
  
  # Mid-Atlantic
  highlight.sector(sector.index = c("NY", "NJ", "PA"), track.index = 1,
                   text = "Mid-Atlantic", padding = c(1,0,5,0), cex = 1.4*font_size, font = 2,
                   border = NA, col = NA, facing = "bending.inside",
                   text.vjust = -1)
  
  # East North Central
  highlight.sector(sector.index = c("IL", "IN", "MI", "OH", "WI"), track.index = 1,
                   text = "East North Central", padding = c(1,0,5,0), cex = 1.4*font_size,
  ...
  highlight.sector(sector.index = c("DE", "FL", "GA", "MD", "NC", "SC", "VA", "DC", "WV"),
                   ...
                   cex = 1.4*font_size, font = 2, border = NA, col = NA, facing = "bending.outside")
  ...
  highlight.sector(sector.index = c("AL", "KY", "MS", "TN"), track.index = 1,
                   text = "East South Central", padding = c(1,0,5,0), cex = 1.4*font_size, font = 2,
                   border = NA, col = NA, facing = "bending.outside",
                   text.vjust = 1.65)
  
  # West South Central
  highlight.sector(sector.index = c("AR", "LA", "OK", "TX"), track.index = 1,
                   text = "West South Central", padding = c(1,0,5,0), cex = 1.4*font_size, font = 2,
                   border = NA, col = NA, facing = "bending.outside")
  
  # Mountain
  highlight.sector(sector.index = c("AZ", "CO", "ID", "MT", "NV", "NM", "UT", "WY"),
                   track.index = 1, text = "Mountain", padding = c(1,0,5,0), cex = 1.4*font_size,
                   font = 2, border = NA, col = NA, facing = "bending.inside")
  
  # Pacific
  highlight.sector(sector.index = c("CA", "OR", "WA", "AK", "HI"), track.index = 1,
                   text = "Pacific", padding = c(1,0,5,0), cex = 1.4*font_size,
